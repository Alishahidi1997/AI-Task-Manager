"""
Run the LLM batch worker (daily summaries, low-priority jobs):

    python -m app.worker.batch_main
...
from app.queue.config import QUEUE_BATCH, rabbitmq_url
...
def main() -> int:
    ...
    channel.basic_consume(queue=QUEUE_BATCH, on_message_callback=_on_message)
    logger.info("batch worker listening on queue=%s", QUEUE_BATCH)
    try:
